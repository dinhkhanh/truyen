...
$books = get_books_search($_GET['letter'], $_GET['tg_id'], $_GET['loai_id']);
if($books){
	echo '<ul class="items">';
	foreach ($books as $book):
    $itacgia = get_author_by_id($book->tg_id);
		echo "<li class='item pull-left'>
				<a href='#' class='apopover' data-placement='right' data-content='{$book->excerpt}' data-title='{$itacgia->ten}' rel='clickover'>
	...
	echo '<p class="alert alert-error">Không tìm thấy kết quả phù hợp.</p>';
}